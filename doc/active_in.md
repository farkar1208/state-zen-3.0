...
- **工厂模式**：使用 `ActiveInFactory` 结构体专门负责创建运行时谓词，职责分离
- **编译**：`ActiveIn::from_blueprint` 将蓝图编译为运行时闭包
- **组合**：支持 AND、OR、NOT 等逻辑组合操作
...
**方法：**
- `pub fn always() -> ActiveIn` - 创建总是为真的谓词
- `pub fn never() -> ActiveIn` - 创建总是为假的谓词
- `pub fn aspect_bool(aspect_id: AspectId, value: bool) -> ActiveIn` - 布尔值等于
- `pub fn aspect_eq(aspect_id: AspectId, value: i64) -> ActiveIn` - 整数等于
- `pub fn aspect_lt(aspect_id: AspectId, value: i64) -> ActiveIn` - 小于
- `pub fn aspect_gt(aspect_id: AspectId, value: i64) -> ActiveIn` - 大于
- `pub fn aspect_in_range(aspect_id: AspectId, min: i64, max: i64) -> ActiveIn` - 范围内
- `pub fn aspect_string_eq(aspect_id: AspectId, value: impl Into<String> + Clone) -> ActiveIn` - 字符串等于
- `pub fn aspect_lt_typed<T>(aspect_id: AspectId, value: T) -> ActiveIn where T: std::cmp::PartialOrd + Send + Sync + 'static` - 泛型小于
- `pub fn aspect_gt_typed<T>(aspect_id: AspectId, value: T) -> ActiveIn where T: std::cmp::PartialOrd + Send + Sync + 'static` - 泛型大于
- `pub fn aspect_eq_typed<T>(aspect_id: AspectId, value: T) -> ActiveIn where T: std::cmp::PartialEq + Send + Sync + 'static` - 泛型等于
- `pub fn and(left: ActiveIn, right: ActiveIn) -> ActiveIn` - 逻辑与
- `pub fn or(left: ActiveIn, right: ActiveIn) -> ActiveIn` - 逻辑或
- `pub fn all(predicates: Vec<ActiveIn>) -> ActiveIn` - 多个谓词的与
- `pub fn any(predicates: Vec<ActiveIn>) -> ActiveIn` - 多个谓词的或
- `pub fn not(predicate: ActiveIn) -> ActiveIn` - 逻辑非
...
1. **性能考虑**：`ActiveIn::from_blueprint` 在每次评估时都会递归调用 `evaluate_blueprint`，对于复杂的谓词可能有性能开销。可以考虑在编译时将 AST 转换为更高效的闭包结构。

2. **错误处理**：当状态中不存在指定的 aspect_id 时，当前行为是返回 `false`。这可能隐藏配置错误。建议考虑添加验证机制或文档说明此行为。

3. **类型支持**：当前只支持 `bool`、`i64`、`String` 三种类型的特定谓词，但提供了泛型方法。建议在文档中更清晰地说明何时使用泛型方法。

4. **文档注释**：部分公开 API 缺少 Rust doc 注释（`///`），建议补充以便生成更好的文档。

5. **命名**：`evaluate_blueprint` 是 `pub(crate)`，但可能对测试和调试有用。考虑是否应该暴露为公开 API 或提供更好的调试接口。